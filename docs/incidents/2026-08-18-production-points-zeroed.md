...
2. **自定义积分项 / 自定义成长项目**：前端已上线，但对应数据库迁移（`supabase/migrations/20260814*_growth_loop_*.sql`）从未经 Shadow Portal 控制面登记、审批、执行，云同步全部失败重试。
...
3. **共享 Supabase proposal-only 流程未走完**：growth_loop 迁移只在本地验证过，从未提交 Shadow Portal 控制面提案审批。
...
- **积分恢复第一步**：合入 PR #44（期初积分受控 RPC + 前端确认流程）。

## 后续行动（进行中）

1. growth_loop 迁移（含期初余额）走 Shadow Portal 控制面提案、审批、执行，生产 schema 就绪后再发布包含 Growth Loop 的版本。
