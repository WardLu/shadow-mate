...
**状态：实现已接入并修复中文音素路径，待 Preview 和真实设备验收**
**日期：2026-09-02**
...
- 语言：简体中文 `zh_CN`。
- 使用拼音音素。现有 `piper-tts-web` 浏览器运行时只提供 eSpeak 音素转换，不能直接兼容该模型；中文必须经 `pinyin-pro` 生成声母、韵母和声调音素后再交给 ONNX 推理。
...
`loadEngine()` 只负责加载一次浏览器 Piper 运行时。`speakLocally(text, packageId)` 从资源注册表取得对应 voice base URL，并让同一个 `PiperWebEngine` 生成目标语言音频。注入引擎的混合音素运行时按模型元数据分流：`phoneme_type: "pinyin"` 使用固定版本的 `pinyin-pro` 生成声母、韵母和声调；英语等其他模型继续使用 bundled eSpeak。两条路径都必须按模型 `phoneme_id_map` fail closed。引擎加载失败与模型下载失败必须分开提示。
...
- 使用模型官方运行时生成的单字、短句和多音字试听样本，以及产品负责人对发音与听感的明确确认；自动生成 WAV、音素或模型加载成功不能替代试听验收。
- 两个文件的实际字节数和 SHA-256。
- 来源、模型卡、数据集许可证和分发说明。
...
已完成 `chaowen` 上游文件、元数据、来源和文件指纹预检；并已用官方 Piper CLI 生成“花”的试听样本，由产品负责人确认发音正确。仍需完成 Preview 浏览器合成、移动端内存观察和真实设备记录。任何剩余门禁失败时，关闭中文下载并保留系统 TTS 回退。
...
已将中文包接入系统 TTS 兜底，复用英语的播放、下载、校验和缓存流程；并修复将 Pinyin 模型误送入 eSpeak 的音素不兼容问题。桌面真实模型合成已核对 `火` 的音素与音素 ID，仍需在 Preview 的小米自带浏览器与 Chrome 上完成实际听感、首次合成耗时和离线重开验收。
...
5. Phase 0 的中文模型先经官方运行时生成可试听的单字、短句和多音字样本，并由产品负责人明确确认发音与听感；随后在实际浏览器中合成目标字和短句。测试同时断言代表性文字的精确 Pinyin 音素与音素 ID，而不是只检查生成了 WAV，且许可证/来源门禁有记录。
