...
5. At runtime, the client resolves the speech ID from the manifest, fetches the immutable object from `voice.shadow.wang`, verifies a successful audio response, and plays it. Standard browser HTTP caching provides repeat reuse. Playback is amplified through the Web Audio API with an input-leveling compressor (-20 dBFS threshold, 2.5:1 ratio), a 3000 Hz peaking filter (+2.5 dB) for vocal clarity, a post-leveler scalable gain multiplier (SPEECH_GAIN_MULTIPLIER = 2.5x delivering 1.5x at 60%, 2.5x / +8 dB at 100%, and up to 5.0x / +14 dB at 200% excess boost), and a safety peak limiter (-0.5 dBFS) to ensure true linear decibel scaling across the slider while protecting against hardware clipping, respecting the device-local `speechVolume` setting. Decoded audio buffers are cached in memory for zero-latency repeat playback.
