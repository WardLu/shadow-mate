...
5. At runtime, the client resolves the speech ID from the manifest, fetches the immutable object from `voice.shadow.wang`, verifies a successful audio response, and plays it. Standard browser HTTP caching provides repeat reuse. Playback is amplified through the Web Audio API with a scalable gain multiplier (5.0x / +14 dB at 100%, 60% default volume delivering 3.0x / +9.5 dB baseline gain, and up to 200% excess boost delivering 10.0x / +20 dB), a 3000 Hz peaking filter (+3.5 dB) for vocal clarity, and a broadcast-grade dynamic range compressor (-12 dBFS threshold, 4:1 ratio) to maximize loudness on mobile device speakers without clipping, while respecting the device-local `speechVolume` setting. Decoded audio buffers are cached in memory for zero-latency repeat playback.
