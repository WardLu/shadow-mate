...
- Package payload: the release's unchanged `sherpa-onnx-wasm-main-tts.data` plus a reproducible mobile-compatible derivative of its WASM. The derivative lowers only the shared-memory import minimum from 512 MB to 256 MB; the JavaScript adapter and worker remain pinned to the same release lineage.
