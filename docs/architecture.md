...
Lucas 学习台适合继续采用“静态 PWA + Supabase + GitHub + Vercel”，但不应照搬旧设计中的 `profiles.parent_id + 12 张内容/用户表`。

本次实施采用两层租户模型：

1. **产品租户**：复用现有 `public.projects`，以 `lucas-learning` 登记学习台，不改现有商业化系统的 `users / subscriptions / credits`。
...
- 新表统一使用 `learning_` 前缀；
- 将来若复用付费能力，再以 `auth.users.id + project_id='lucas-learning'` 查询订阅，不让付费表成为学习数据的所有者。
...
当前 `public/config.js` 中的 Supabase publishable key 可公开。RLS 是数据安全边界。禁止提交 secret/service role key。

### 域名切换顺序

1. 部署 Preview。
2. 验证登录回跳、家庭创建、跨设备同步和离线启动。
3. 将生产域名加入 Supabase redirect allowlist。
4. 绑定 Vercel 自有域名。
5. 再将 Supabase Site URL 切到生产域名。

## 10. 验证结果与运维

数据库已验证：

- 产品 `lucas-learning` 已登记；
- 4 张 `learning_*` 表均已启用 RLS；
- 共 12 条针对 authenticated 的策略；
- `learning_save_state` 为 security invoker；
- 使用无 membership 的伪 authenticated 用户查询，所有学习表可见行均为 0；
- Supabase Security Advisor 对 `learning_*` 无告警；
- Performance Advisor 提示的两个缺失外键索引已补齐；
- 新索引尚未使用属于空表初始化阶段的正常信息。
