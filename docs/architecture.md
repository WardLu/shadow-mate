...
- **Unauthenticated mode (single device, local only)**: With no remote household (`household_id === null`), redemptions and refunds are authoritative on the local device. Redeeming a reward immediately creates a confirmed redemption and debits the ledger, allowing parents to directly mark "fulfilled", or cancel (both before or after fulfillment to recover from misoperations) to immediately refund points without waiting for network connectivity.
