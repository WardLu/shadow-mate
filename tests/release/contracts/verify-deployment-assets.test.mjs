...
import {verifyAssets,verifyProductionAliases} from '../../../scripts/release/verify-deployment-assets.mjs';
...
test('unchanged JS/CSS cannot hide a stale production alias',()=>{
 const config={vercel:{projectId:'prj_fixture',teamId:'team_fixture',productionDomains:['example.invalid']}};
 for(const stale of [false,true]) {
  const run=(_cmd,args)=>({status:0,stdout:JSON.stringify(args[1].startsWith('/v13/')?{id:'dpl_new',projectId:'prj_fixture'}:{deploymentId:stale?'dpl_old':'dpl_new',projectId:'prj_fixture'})});
  const verify=()=>verifyProductionAliases({config,candidateUrl:'https://candidate.vercel.app',run});
  if(stale)assert.throws(verify,/production_alias_mismatch/);else assert.doesNotThrow(verify);
 }
});
