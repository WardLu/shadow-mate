import { createClient } from "https://esm.sh/@supabase/supabase-js@2.111.0?bundle";
import { CLOUD_CONFIG } from "/config.js";

const PRODUCT_ID = CLOUD_CONFIG.productId;
const ACTIVE_PROFILE_KEY = "lucas_learning_active_profile";
...
function renderSetup() {
  panel.innerHTML = `
    ...
    <form id="householdSetupForm">
      <label class="cloud-field">
        家庭空间名称
        <input name="household" maxlength="40" required value="Lucas 的家庭">
      </label>
      <label class="cloud-field">
        第一个学习者
        <input name="learner" maxlength="30" required value="Lucas">
      </label>
      <label class="cloud-field">
        当前年级
        <select name="grade">
...
async function loadWorkspace(preferredProfileId = null, options = {}) {
  ...
  const remembered = preferredProfileId || localStorage.getItem(ACTIVE_PROFILE_KEY);
