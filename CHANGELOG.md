...
### Added
- 页脚增加 `support@shadow.wang` 问题反馈入口。
- 接入无自定义属性的最小产品事件，包括首次激活、首次打卡、连续三天打卡、学习记录同步失败和发音失败；同步更新中英文隐私说明。

### Fixed
- 首次打卡事件排除撤销操作；连续打卡判断排除取消记录和无效日期。

### Tests
- 补齐登录态字帖无“第一单元”断言，以及字帖轮换的同学习者合并、跨学习者隔离回归。
