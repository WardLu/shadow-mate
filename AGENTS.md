...
- 纯逻辑或 UI 小改：运行 `npm run check`、相关 `npm run test:unit`，并实际检查受影响页面和移动视口。
- 状态、认证、同步或生命周期改动：增加失败场景测试，并按范围运行 `npm run test:unit`、`npm run test:db`、`npm run test:functions` 或 `npm run test:e2e`。
- 构建、PWA、CSP、依赖或公开页面改动：运行 `npm run verify`；注意它不包含数据库、Functions 和 E2E 测试。
- 发布候选再运行 `npm run release:check` 并核对真实构建产物；这不授权发布或部署。

## 权威入口

- 架构与数据模型：`docs/architecture.md`
- 安全基线：`docs/security-baseline.md`
- 内部工程架构与共享 Supabase 验收：`/Users/wardlu/Documents/Ward's Obsidian/01_Project/Apps/Shadow Mate/07-工程架构与共享 Supabase 验收.md`
- 内部测试范围与验收门槛：`/Users/wardlu/Documents/Ward's Obsidian/01_Project/Apps/Shadow Mate/08-测试范围与验收门槛.md`
