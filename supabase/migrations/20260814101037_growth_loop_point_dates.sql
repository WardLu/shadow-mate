-- Preserve the date selected in the local-first calendar when a point event
-- reaches the cloud. The original six-argument RPC remains for compatibility;
-- the seven-argument overload is used by the Growth Loop client.

alter table public.learning_point_ledger
  add column if not exists occurred_on date not null default current_date;

create index if not exists learning_point_ledger_profile_occurred_idx
  on public.learning_point_ledger (profile_id, occurred_on desc);
