...
-- learning_ prefix and the product registry key `shadow-mate`.
...
create table if not exists public.learning_households (
  ...
  project_id text not null default 'shadow-mate'
...
-- The member insert policy must not query learning_households directly as the
-- invoking user: that table's read policy checks memberships and would create
-- an RLS recursion loop. This narrowly scoped owner check runs as the migration
-- owner, uses a fixed search path, and returns no household data.
...
revoke all on function private.learning_is_household_owner(uuid) from public;
revoke all on function private.learning_is_household_owner(uuid) from anon;
